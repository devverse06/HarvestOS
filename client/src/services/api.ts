...
// --- Quality, Warehouse, and Inventory API ---
export const createQualityInspection = async (inspectionData: any) => {
  const response = await fetch(`${API_URL}/quality-inspections`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(inspectionData) });
  if (!response.ok) throw new Error('Failed to create quality inspection');
  return response.json();
};
export const getWarehouses = async () => {
  const response = await fetch(`${API_URL}/warehouses`, { headers: getAuthHeaders() });
  if (!response.ok) throw new Error(`API ${response.status}`);
  return response.json();
};
export const createWarehouse = async (warehouseData: any) => {
  const response = await fetch(`${API_URL}/warehouses`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(warehouseData) });
...
export const createInventoryBatch = async (batchData: any) => {
  const response = await fetch(`${API_URL}/inventory`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(batchData) });
  if (!response.ok) throw new Error('Failed to create inventory batch');
  return response.json();
};

// --- Orders and Analytics API ---
...
export const createOrder = async (orderData: any) => {
  const response = await fetch(`${API_URL}/orders`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(orderData) });
  if (!response.ok) {
    const errorData = await response.json();
...
export const cancelOrder = async (id: string) => {
  const response = await fetch(`${API_URL}/orders/${id}/cancel`, { method: 'PATCH', headers: getAuthHeaders() });
  if (!response.ok) throw new Error('Failed to cancel order');
  return response.json();
};
export const getTraceability = async (lotId: string) => {
  const response = await fetch(`${API_URL}/traceability/${lotId}`, { headers: getAuthHeaders() });
  if (!response.ok) throw new Error(`API ${response.status}`);
  return response.json();
};
