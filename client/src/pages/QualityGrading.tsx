...
import { createQualityInspection, getHarvestLots } from '../services/api';
...
export default function QualityGrading() {
  ...
  // Chemical values are entered by the operator; the backend stores the disposition.
  const [testResults, setTestResults] = useState({
    'Chlorpyrifos': null as number | null,
    'Carbendazim': null as number | null,
    'Imidacloprid': null as number | null,
    'Mancozeb': null as number | null
  });
  ...
  const [physicalGrades, setPhysicalGrades] = useState({
    ...
    coloration: null as number | null,
    firmness: null as number | null,
    defects: null as number | null
  });
  ...
  const complianceStatus = Object.keys(testResults).map(chemical => {
    ...
    const isPass = actual !== null && actual <= limit;
    return { chemical, limit, actual, isPass };
  });

  const allChemicalsPass = complianceStatus.every(c => c.isPass);
  const physicalPass = physicalGrades.brix !== null && physicalGrades.defects !== null && physicalGrades.brix >= 15 && physicalGrades.defects <= 5;
  ...
  const handleApprove = async () => {
    ...
    try {
      await createQualityInspection({
        id: `QI-${selectedLot.id}`,
        lot_id: selectedLot.id,
        grade: physicalPass ? 'A' : 'Rejected',
        passed: allChemicalsPass && physicalPass,
        notes: `Destination: ${destination}`,
      });
      setIsApproving(false);
      setIsApproved(true);
    } catch (err) {
      console.error('Failed to save quality inspection', err);
      setIsApproving(false);
    }
  };
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[14px] text-slate-500">Enter measured values and persist the inspection disposition.</p>
        </div>
            ...
            <div className="space-y-5">
              <div>
                <div className="flex justify-between text-sm mb-1.5">
                  ...
                  <span className={`font-bold ${physicalGrades.brix !== null && physicalGrades.brix < 15 ? 'text-red-500' : 'text-slate-500'}`}>{physicalGrades.brix === null ? '—' : `${physicalGrades.brix}%`}</span>
                </div>
                <input
                  type="range" min="10" max="25" step="0.5"
                  value={physicalGrades.brix ?? ''}
                  onChange={(e) => setPhysicalGrades(p => ({ ...p, brix: e.target.value === '' ? null : parseFloat(e.target.value) }))}
                  className="w-full accent-blue-600"
                  disabled={isApproved}
                />
                {physicalGrades.brix !== null && physicalGrades.brix < 15 && <p className="text-[11px] text-red-500 mt-1">Below target export threshold (15%).</p>}
              </div>

              <div>
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="font-medium text-slate-700">Defects & Blemishes</span>
                  <span className={`font-bold ${physicalGrades.defects !== null && physicalGrades.defects > 5 ? 'text-red-500' : 'text-slate-700'}`}>{physicalGrades.defects === null ? '—' : `${physicalGrades.defects}%`}</span>
                </div>
                <input
                  type="range" min="0" max="15" step="1"
                  value={physicalGrades.defects ?? ''}
                  onChange={(e) => setPhysicalGrades(p => ({ ...p, defects: e.target.value === '' ? null : parseInt(e.target.value, 10) }))}
                  className="w-full accent-blue-600"
                  disabled={isApproved}
                />
              </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1.5">
                  ...
                  <span className="font-bold text-slate-700">{physicalGrades.coloration === null ? '—' : `${physicalGrades.coloration}%`}</span>
                </div>
                <input
                  type="range" min="50" max="100" step="5"
                  value={physicalGrades.coloration ?? ''}
                  onChange={(e) => setPhysicalGrades(p => ({ ...p, coloration: e.target.value === '' ? null : parseInt(e.target.value, 10) }))}
                  className="w-full accent-blue-600"
                  disabled={isApproved}
                />
              </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex items-center justify-between">
            ...
            {!isApproved && (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-semibold text-sm transition-all ${canApprove
