...
import { getFarms, getHarvestLots, getShipments, createShipment, getOrders } from '../services/api';
...
export default function Dashboard() {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.order_id) {
      alert("Please select a reserved order before creating a shipment.");
      return;
    }
    ...
    try {
      ...
      setFormData({ id: '', order_id: '', container_number: '' });
            ...
            <div className="px-5 py-2 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2 text-[11px]">
              ...
              {['All', 'Intake', 'Quality', 'Packing', 'Storage'].map(s => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-full font-medium transition-colors ${(s === 'All' && statusFilter === 'filter') || statusFilter === s
            ...
            >
              ...
              <form onSubmit={handleCreateSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[12px] font-medium text-slate-700">Reserved Order</label>
                    <select required value={formData.order_id} onChange={(e) => setFormData({ ...formData, order_id: e.target.value })} className="w-full px-3 py-2 text-[13px] border border-slate-300 rounded-md focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary bg-white">
                      <option value="">Select an Order...</option>
                      {orders.filter((order) => order.status === 'Reserved').map((order) => (
                        <option key={order.id} value={order.id}>{order.id} ({order.buyer_id})</option>
                      ))}
                    </select>
                  </div>
                </div>
